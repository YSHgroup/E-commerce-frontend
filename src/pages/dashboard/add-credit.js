...
const AddCredit = ({ user }) => {
    ...
    useEffect(()=>{
        ...
        .then((res) => {
            const {email, first_name, last_name, address, city, phone, state, postal_code, company, country: c, vat_id} = res.data;
            setCountry(c);
            if(c) {
                setRate(taxRate[`${c}`] ?? 0);
            }
    ...
    useEffect(() => {
        ...
        const addSBL = () => {
            ...
            if (!existingScript) {
              const storeFrontToUse = "3dinfinite.onfastspring.com/popup-pop-add-credit";
